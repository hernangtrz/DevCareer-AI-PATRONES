...
export class CvOptimizationService {
   ...
   */
  async improveCv(input: ImproveCvInput): Promise<ImprovedCvResult> {
...
Devuelve ÚNICAMENTE un objeto JSON estructurado exactamente así:
{
  ...
  "skills": [
    ...
    return this.aiProvider.generateJson<ImprovedCvResult>(
      [{ text: prompt }],
      { temperature: 0.4 }
    );
  }
   ...
   */
  async improveProfile(input: ImproveProfileInput): Promise<ImprovedProfileResult> {
...
Devuelve ÚNICAMENTE un objeto JSON estructurado exactamente así:
{
  "refinedProfile": "Párrafo de perfil profesional mejorado en el idioma solicitado"
    ...
    return this.aiProvider.generateJson<ImprovedProfileResult>(
      [{ text: prompt }],
      { temperature: 0.5 }
    );
  }
